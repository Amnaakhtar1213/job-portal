...
const JobsCard = ({job}) => {
  ...
  <span className="hover:-translate-y-1 transition-all duration-300 px-3 py-1 text-sm bg-blue-100 text-blue-700 rounded-full">
    {job.type}
  </span>
  ...
  <span className="hover:-translate-y-1 transition-all duration-300 px-3 py-1 text-sm bg-purple-100 text-purple-700 rounded-full">
    {job.salary}
  </span>
...
<button onClick={() => navigate(`/loggin/${job.id}`)} className="group flex flex-row gap-4 items-center justify-center bg-blue-700 hover:bg-blue-800 text-white px-6 rounded-2xl py-1 mt-10 hover:-translate-y-1 transition transform duration-300 ease-in-out tracking-wide [text-shadow:1px_1px_3px_rgba(0,0,0,0.3)] animate-fadeIn"><span>Apply</span><i className="fa-solid fa-arrow-right group-hover:animate-[leftRight_2s_ease-in-out_infinite]"></i></button>
</div>
    </div>
  )
}
